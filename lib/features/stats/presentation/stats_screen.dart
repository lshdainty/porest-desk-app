...
/// 히트맵 셀 라벨 — 축약은 차트 축·도넛 중앙과 **같은 함수**([formatChartAxis]).
///
/// 예전엔 1만 아래를 `5천` 으로 한 번 더 줄였다. 칸이 좁아서였는데, `천` 은 합의한
/// 단위가 아니다 — 단위는 만·억·조 뿐이고 1만 미만은 `4,900` 처럼 천단위 콤마
/// 정수로 낸다(QA #73). 한 화면에서 도넛 중앙은 `4,900`, 그 옆 히트맵은 `5천` 이면
/// 같은 값이 두 글자로 보인다. 칸 폭은 셀의 `FittedBox` 가 글자를 줄여 해결한다.
