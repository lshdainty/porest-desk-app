...
/// 전용으로 둔다(D15). 서버도 값이 바뀌면 400 이다.
 bool get carryoverLocked;/// 신용카드: **이 날짜(`yyyy-MM-dd`) 이하 거래는 결제가 끝난(닫힌) 회차**다.
///
...
/// 카드·신용카드가 아닌 자산·옛 서버면 null 이다(그때는 모두 열린 회차로 본다).
 String? get cardClosedThrough;// 연결된 카드 상품 (카드 자산 전용, nullable) — 편집 진입 시 선택 상태 복원용.
...
@useResult
$Res call({
 int rowId, int? userRowId, String assetName, String assetType, int? balance, int? cashBalance, int? holdingBalance, String? currency, double? exchangeRate, String? color, String? institution, String? memo, int? sortOrder, String? isIncludedInTotal, String? isAmountHidden, int? creditLimit, int? paymentDay, int? paymentAssetRowId, int? carryoverAmount, bool carryoverLocked, String? cardClosedThrough, AssetCardCatalog? cardCatalog, String? marketCode, String? tossSymbol, int? tossQuantity, List<AssetHolding> holdings, int? monthlyUsedAmount
});
...
as String?,isAmountHidden: freezed == isAmountHidden ? _self.isAmountHidden : isAmountHidden // ignore: cast_nullable_to_non_nullable
...
as bool,cardClosedThrough: freezed == cardClosedThrough ? _self.cardClosedThrough : cardClosedThrough // ignore: cast_nullable_to_non_nullable
...
switch (_that) {
case _Asset() when $default != null:
return $default(_that.rowId,_that.userRowId,_that.assetName,_that.assetType,_that.balance,_that.cashBalance,_that.holdingBalance,_that.currency,_that.exchangeRate,_that.color,_that.institution,_that.memo,_that.sortOrder,_that.isIncludedInTotal,_that.isAmountHidden,_that.creditLimit,_that.paymentDay,_that.paymentAssetRowId,_that.carryoverAmount,_that.carryoverLocked,_that.cardClosedThrough,_that.cardCatalog,_that.marketCode,_that.tossSymbol,_that.tossQuantity,_that.holdings,_that.monthlyUsedAmount);case _:
  return orElse();
...
// 'Y' | 'N'
/// 이 자산의 금액만 가린다. 화면 카드 가리기와 별개 축, 합집합.
@override final  String? isAmountHidden;
// 'Y' | 'N'
// 신용카드 청구 사이클 (CREDIT_CARD 전용, nullable).
...
/// 지금 총 미결제 잔액([balance])이 아니다 — 그걸로 채워 저장하면 이월 거래가 잔액만큼
/// 새로 생겨 빚이 두 배가 됐다(QA 23차 1). 신용카드가 아니거나 옛 서버면 null.
@override final  int? carryoverAmount;
...
/// 시계로 정한다(D5·D11). 닫힌 회차에 걸린 변경은 기록만 바뀐다(D1). 결제일 없는
/// 카드·신용카드가 아닌 자산·옛 서버면 null 이다(그때는 모두 열린 회차로 본다).
@override final  String? cardClosedThrough;
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,rowId,userRowId,assetName,assetType,balance,cashBalance,holdingBalance,currency,exchangeRate,color,institution,memo,sortOrder,isIncludedInTotal,isAmountHidden,creditLimit,paymentDay,paymentAssetRowId,carryoverAmount,carryoverLocked,cardClosedThrough,cardCatalog,marketCode,tossSymbol,tossQuantity,const DeepCollectionEquality().hash(_holdings),monthlyUsedAmount]);
