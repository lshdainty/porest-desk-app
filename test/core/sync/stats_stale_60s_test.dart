...
//
// 통계 provider 넷(`rangeSummary`·`rangeExpenses`·`heatmap`·`merchantSummary`)은
...
/// 통계 4종의 조회 횟수 — 진입 갱신이 **재조회로 이어졌는지** 보는 자다.
...
void main() {
  testWidgets('60초 안에 다시 들어가면 통계 4종을 다시 받지 않는다', (tester) async {
  ...
  testWidgets('60초가 지나 들어가면 통계 4종을 각각 다시 받는다', (tester) async {
    final h = await _pump(tester);
...
/// 통계 화면이 읽는 자리를 대신한다 — watch 하지 않으면 무효화해도 재조회가 없다.
...
Future<_Harness> _pump(WidgetTester tester) async {
  ...
  await tester.pumpWidget(
    ProviderScope(
      ...
      child: MaterialApp(
        home: _RefProbe(onRef: (r) => captured = r, child: const _WatchStats()),
      ),
    ),
  );
  await tester.pumpAndSettle();
